function [ cParams, sDefaultPhase ] = gas(oVsys, fVolume, tfPartialPressure, fTemperature, rRelativeHumidity)
%Gas helper to create a gas matter phase by defining the partial pressures 
% of the components and the temperature.
%
% gas Parameters:
%   oVsys               - can be a vsys but also a store, basically
...
fRm           = oVsys.oMT.Const.fUniversalGas;                 % ideal gas constant [J/K]
...
mfPartialPressure = zeros(1, oVsys.oMT.iSubstances);
...
for iSubstance = 1:length(csSubstances)
    mfPartialPressure(oVsys.oMT.tiN2I.(csSubstances{iSubstance})) = tfPartialPressure.(csSubstances{iSubstance});
end
...
if rRelativeHumidity > 0
    fSaturationVaporPressureWater = oVsys.oMT.calculateVaporPressure(fTemperature, 'H2O');
    % relative humidity is defined as the percentage of vapor pressure at the
    % current vapor pressure
    mfPartialPressure(oVsys.oMT.tiN2I.H2O) = rRelativeHumidity * fSaturationVaporPressureWater;
end
...
%     tParameters = struct();
%     tParameters.sSubstance      = oVsys.oMT.csSubstances{aiIndices(iIndex)};
...
%     afRho(iIndex) = oVsys.oMT.findProperty(tParameters);
% end

% Now we use the ideal gas law to calculate the mass of the individual
% components:
% p V = m R T --> m = (p V) / (R T)
afMass = (mfPartialPressure .* fVolume) ./ ((fRm ./ oVsys.oMT.afMolarMass) .* fTemperature);

% define the struct that is required to define the phase in the vsys:
tfMass = struct();
for iIndex = 1:length(aiIndices)
    tfMass.(oVsys.oMT.csSubstances{aiIndices(iIndex)}) = afMass(aiIndices(iIndex));
end

% To make the function easier to use for different use cases we
% differentiate between the case where the store create phase function is
% used to generate the desired gas phase and all other cases. If not the
% store calls, the function will simply provide the tfMass struct directly,
% as the user most likely is not interessted in receiving the inputs he
% made as outputs again. The default phase is just added as output but does
% not have to be used in this case.
toCallStack = dbstack;
if strcmp(toCallStack(2).file, 'store.m')
