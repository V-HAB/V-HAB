function fSpecificHeatCapacity = calculateHeatCapacity(this, varargin) %sMatterState, afMasses, fTemperature, fPressure)
    ...
    if iNumArgs == 1 %nargin < 3
        ...
        elseif isa(oMatterRef, 'matter.flow')
            ...
            % Because a flow can be negative, we transform the afMasses
            % array into absolutes here. Otherwise the calculated heat
    ...
    %DEBUG
    assert(isequal(size(afCp), size(afMasses(aiIndices)')), 'Vectors must be of same length but one transposed.');
    ...
    % the mixture.
    fSpecificHeatCapacity = (afMasses(aiIndices) ./ sum(afMasses)) * afCp;
