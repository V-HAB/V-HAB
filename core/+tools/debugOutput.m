classdef debugOutput < handle
    ...
    methods
        function output(this, oObj, iLevel, iVerbosity, sIdentifier, sMessage, cParams)
            ...
            % Getting caller method name from stack
            tStack   = dbstack(2);
            csMethod = strsplit(tStack(1).name, '.');
            
            % If the method has been moved into a separate file to keep the
            % overall file size down, the name will not have a dot ('.')
            % character. The following code catches this case.
            try 
                sMethod = csMethod{2};
            catch
                sMethod = csMethod;
