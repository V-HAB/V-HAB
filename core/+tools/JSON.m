classdef JSON
    ...
    properties (Constant, Access = private)
        JARFILE = tools.JSON.jarfile % Java class path to JSON library
    end
    
    methods (Static)        
        function X = load( S )
            %LOAD load matlab object from json string
            %CL only do if not yet there ...
            if ~any(cell2mat(regexp(javaclasspath(), 'json\.jar')))
                javaaddpath(tools.JSON.JARFILE);
            end
            ...
            X = tools.JSON.load_data(obj);
        end
        
        function S = dump( X )
            %CL check if it works without add - already added
            try
                obj = tools.JSON.dump_data(X);
            catch %#ok<CTCH>
    ...
    methods (Static, Access=private)
        ...
        function result = load_data( r )
            ...
            elseif isa(r, 'org.json.JSONArray')
                ...
                for i = 1:r.length()
                    result{i} = tools.JSON.load_data(r.get(i-1));
                end
                result = tools.JSON.merge_cell(result);
            elseif isa(r, 'org.json.JSONObject')
                %CL use containers.Map instead of useless structs!
                result = containers.Map();%struct;
                itr = r.keys();
                while itr.hasNext()
                    key = itr.next();
                    result(char(key)) = tools.JSON.load_data(...
                        r.get(java.lang.String(key)));
                end
            %CL include logical support
        ...
        function result = dump_data( r )
            ...
            elseif isa(r, 'containers.Map')
                ...
                keys = r.keys();
                for i = 1:length(keys)
                    result.put(keys{i},tools.JSON.dump_data(r(keys{i})));
                end
            elseif ~isscalar(r)
                ...
                if size(r,1)==1
                    for i = 1:numel(r)
                        if iscell(r)
                            result.put(tools.JSON.dump_data(r{i}));
                        else
                            result.put(tools.JSON.dump_data(r(i)));
                        end
                    end
            ...
            elseif iscell(r)
                result = org.json.JSONArray();
                result.put(tools.JSON.dump_data(r{1}));
            else
                error('JSON:load_data:typeError',...
