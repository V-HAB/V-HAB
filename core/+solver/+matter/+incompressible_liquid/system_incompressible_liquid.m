                ...
                for Step = 1:this.iPartSteps
                    ...
                    for k = 1:this.iNumberOfBranches
                        try 
                            ...
                            mPressureLossNew(k) = functions.calculateDeltaPressure.Pipe(sqrt(this.mBranchArea(k)/(0.25*pi)), 1/this.mInverseBranchLength(k),...
                                        mFlowSpeedStep(k,Step), mBranchViscosities(k), mNewBranchDensities(k), 0.0002, 0);
                            mPressureLossPipeOld(k) = functions.calculateDeltaPressure.Pipe(sqrt(this.mBranchArea(k)/(0.25*pi)), 1/this.mInverseBranchLength(k),...
