...
function saveFigureAs(hButton,~)
    ...
    % We need to change a property of the axes object before modifying
    % the font size. To do this, we need to find out, which one of the
    % children is the axes object.
    for iI = 1:iChildren
    ...
    % Again, we're saving everything to restore it later.
    iLabelFontSizeMultiplier = hFigure.Children(iAxesIndex).LabelFontSizeMultiplier;
    hFigure.Children(iAxesIndex).LabelFontSizeMultiplier = 1;
    
    % Now we can go throuhg all objects in the figure, save their
    ...
    if ~bTitleOn && strcmp(hFigure.Children(iAxesIndex).Title.Visible,'on')
        bOldTitleState = true;
        hFigure.Children(iAxesIndex).Title.Visible = 'off';
    elseif bTitleOn && strcmp(hFigure.Children(iAxesIndex).Title.Visible,'off')
        bOldTitleState = false;
        hFigure.Children(iAxesIndex).bOldTitleState = 'on';
    end
    
    % We want to make sure, that the axes really fill out the figure so now
    % we'll change the size of the plot. Since we want to restore the size
    % later, we save it first.
    afOldAxesPosition = hFigure.Children(iAxesIndex).Position;
    
    %TODO Insert a check here if there are two y axes. If so, the position
    %values set below need to be different, so the right side axis labels
    %are not cut off. 
    
    hFigure.Children(iAxesIndex).Units = 'centimeters';
    hFigure.Children(iAxesIndex).Position = [ 1.3 1 fFigureWidth-1.7 fFigureHeight-1.5];
    hFigure.Children(iAxesIndex).Units = 'normalized';
    
    % Check the boolean grid variables to see if the user wants them on or
    % off for this figure. Depending on this variable and the current
    % state, we do the change. We save the state in a boolean variable so
    % we can reset after saving.
    
    % Main Grid
    if ~bMainGrid && strcmp(hFigure.Children(iAxesIndex).YGrid,'on')
        bYMainGrid = true;
        hFigure.Children(iAxesIndex).YGrid = 'off';
    elseif bMainGrid && strcmp(hFigure.Children(iAxesIndex).YGrid,'off')
        bYMainGrid = false;
        hFigure.Children(iAxesIndex).YGrid = 'on';
    end
    
    if ~bMainGrid && strcmp(hFigure.Children(iAxesIndex).XGrid,'on')
        bXMainGrid = true;
        hFigure.Children(iAxesIndex).XGrid = 'off';
    elseif bMainGrid && strcmp(hFigure.Children(iAxesIndex).XGrid,'off')
        bXMainGrid = false;
        hFigure.Children(iAxesIndex).XGrid = 'on';
    end
    
    % Minor Grid
    if bMinorGrid == 0 && strcmp(hFigure.Children(iAxesIndex).YMinorGrid,'on')
        bYMinorGrid = true;
        hFigure.Children(iAxesIndex).YMinorGrid = 'off';
    elseif bMinorGrid && strcmp(hFigure.Children(iAxesIndex).YMinorGrid,'off')
        bYMinorGrid = false;
        hFigure.Children(iAxesIndex).YMinorGrid = 'on';
    end
    
    if ~bMinorGrid && strcmp(hFigure.Children(iAxesIndex).XMinorGrid,'on')
        bXMinorGrid = true;
        hFigure.Children(iAxesIndex).XMinorGrid = 'off';
    elseif bMinorGrid && strcmp(hFigure.Children(iAxesIndex).XMinorGrid,'off')
        bXMinorGrid = false;
        hFigure.Children(iAxesIndex).XMinorGrid = 'on';
    end
    
    % The user may have elected to auto-adjust the axes limits, so we save
    % the old limits and set the mode to auto.
    afOldXLimits = hFigure.Children(iAxesIndex).XLim;
    afOldYLimits = hFigure.Children(iAxesIndex).YLim;
    
    if bAutoAdjustXAxis
        hFigure.Children(iAxesIndex).XLimMode = 'auto';
    end
    if bAutoAdjustYAxis
        hFigure.Children(iAxesIndex).YLimMode = 'auto';
    end
    ...
    hFigure.Children(iAxesIndex).LabelFontSizeMultiplier = iLabelFontSizeMultiplier;
    ...
    if exist('bOldTitleState','var') && bOldTitleState
        hFigure.Children(iAxesIndex).Title.Visible = 'on';
    elseif exist('bOldTitleState','var') && bOldTitleState == 0
        hFigure.Children(iAxesIndex).Title.Visible = 'off';
    end
    
    % Restoring the old axes positions, expanding the axes to fill the
    % figure and restoring the figure to its original size.
    hFigure.Children(iAxesIndex).Position = afOldAxesPosition;
    hFigure.Children(iAxesIndex).OuterPosition = [ 0 0 1 1 ];
    hFigure.Children(iAxesIndex).XLim = afOldXLimits;
    hFigure.Children(iAxesIndex).YLim = afOldYLimits;
...
function [ fFigureWidth, fFigureHeight, sFontName, fFontSize, ...
           bTitleOn, bMainGrid, bMinorGrid, bAutoAdjustXAxes, ...
           bAutoAdjustYAxes ] = settingsDialog()
    ...
    % Creating a checkbox
    oDialog.UserData.oAutoAdjustXAxes = uicontrol('Parent',oPanel,...
              'Style','checkbox',...
              'Units','pixels',...
              'Position',[80 20 50 iLineHeight]);
    ...
    function finish(oButton, ~)
        ...
        bAutoAdjustXAxes = tUserData.oAutoAdjustXAxis.Value;
        bAutoAdjustXAxis_Preset = bAutoAdjustXAxes;
        
        bAutoAdjustYAxes = tUserData.oAutoAdjustYAxis.Value;
        bAutoAdjustYAxis_Preset = bAutoAdjustYAxes;
