function [oCulture] ...
    = PlantGrowth(oCulture, fDensityAtmosphere, fTemperatureAtmosphere, fRelativeHumidityAtmosphere, fHeatCapacityAtmosphere, fDensityH2O)
    ...
    if oCulture.ttxPlantParameters.bLegume == 1
        % before time of onset of canopy senescence
        if fInternalTime <= oCulture.ttxPlantParameters.fT_Q
            fCUE_24 = oCulture.ttxPlantParameters.fCUE_Max;
            % after time of onset of canopy senescence but before time of
            % crop maturity
        elseif oCulture.ttxPlantParameters.fT_Q < fInternalTime <= oCulture.ttxPlantParameters.fT_M
            fCUE_24 = oCulture.ttxPlantParameters.fCUE_Max - (oCulture.ttxPlantParameters.fCUE_Max - oCulture.ttxPlantParameters.fCUE_Min) * (fInternalTime - oCulture.ttxPlantParameters.fT_Q) * (oCulture.ttxPlantParameters.fT_M - oCulture.ttxPlantParameters.fT_Q)^-1;
        end
    ...
    % [µmol m^-2 s-^1]
    fPPFD_E = fPPFD * (fH * oCulture.ttxPlantParameters.fH_0^-1);
    ...
    fT_A = ...
        ...
        oCulture.ttxPlantParameters.mfMatrix_T_A * ...  % coefficient matrix
    ...
    if fInternalTime < fT_A
        fA = oCulture.ttxPlantParameters.fA_Max * (fInternalTime / fT_A)^oCulture.ttxPlantParameters.fN;
    % after time of canopy closure
    elseif fInternalTime >= fT_A
        fA = oCulture.ttxPlantParameters.fA_Max;
    end
    ...
    fCQY_Max = ...
        [1/fCO2 1 fCO2 fCO2^2 fCO2^3] * ...                 % row vector for CO2
        oCulture.ttxPlantParameters.mfMatrix_CQY_Max * ...  % coefficient matrix
    ...
    % after time of onset of canopy senescence but before time of
     % crop maturity    
    elseif oCulture.ttxPlantParameters.fT_Q < fInternalTime <= oCulture.ttxPlantParameters.fT_M
        fCQY = fCQY_Max - (fCQY_Max - oCulture.ttxPlantParameters.fCQY_Min) * (fInternalTime - oCulture.ttxPlantParameters.fT_Q) * (oCulture.ttxPlantParameters.fT_M - oCulture.ttxPlantParameters.fT_Q)^-1;
    end
    
    % hourly carbon gain [mol_Carbon m^-2 h^-1]
    % HCG = alpha * CUE_24 * A * CQY * PPFD * I (Eq. 2)
    fHCG = oCulture.ttxPlantParameters.fAlpha * oCulture.ttxPlantParameters.fCUE_24 * fA * fCQY * fPPFD * bI;
    
    % hourly crop growth rate (dry) [g m^-2 h^-1]
    % HCGR = HCG * MW_C * BCF^-1 (Eq. 6)
    fHCGR = fHCG * oCulture.oMT.ttxMatter.fMolarMass(oMT.tiN2I.C) * oCulture.ttxPlantParameters.fBCF;
    
    % hourly wet crop growth rate [g m^-2 h^-1]
    % HWCGR = HCGR * (1 - WBF)^-1 (Eq. 7)
    fHWCGR = fHCGR * (1 - oCulture.ttxPlantParameters.fWBF)^-1;
    
    % hourly oxygen production [g m^-2 h^-1]
    % HOP = HCG * CUE_24 ^-1 * OPF * MW_O2 (Eq. 8)
    fHOP = fHCG * fCUE_24 * oCulture.ttxPlantParameters.fOPF * oCulture.oMT.ttxMatter.fMolarMass(oMT.tiN2I.O2);
    
    % hourly oxygen consumption [g m^-2 h^-1]
    % HOC = HCG * I^-1 * (1 - CUE_24) * CUE_24^-1 * OPF * MW_O2 * H * 24^-1
    % (Eq. 9)
    fHOC = (oCulture.ttxPlantParameters.fAlpha * fCUE_24 * fA * fCQY * fPPFD) * (1 - fCUE_24) * fCUE_24^-1 * oCulture.ttxPlantParameters.fOPF * oCulture.oMT.ttxMatter.fMolarMass(oMT.tiN2I.O2) * fH * 24^-1;
    ...
    fHNC = fHCGR * oCulture.ttxPlantParameters.fDRY_Fraction * oCulture.ttxPlantParameters.fNC_Fraction; 
    ...
    if fInternalTime < fT_A  
        fKC = fKC_Mid * (fInternalTime / fT_A) ^ oCulture.ttxPlantParameters.fN;
    elseif (fT_A <= fInternalTime) && (fInternalTime <= oCulture.ttxPlantParameters.fT_Q)   
        fKC = fKC_Mid;
    else   
        fKC = fKC_Mid + ((fInternalTime - oCulture.ttxPlantParameters.fT_Q) / (oCulture.ttxPlantParameters.fT_M - oCulture.ttxPlantParameters.fT_Q)) * (fKC_Late - fKC_Mid);
    end
