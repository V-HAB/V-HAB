    ...
    methods
        ...
        function configureMonitors(this)
            ...
            for iStore = 1:length(csStores)
                if strcmp(csStores{iStore}, 'Filter')
                    iPhase = 2;
                else
                    iPhase = 1;
                end
                oLog.addValue(['Example.toStores.', csStores{iStore}, '.aoPhases(',num2str(iPhase),')'],	'this.fMass * this.fMassToPressure',	'Pa', [csStores{iStore}, ' Pressure']);
                oLog.addValue(['Example.toStores.', csStores{iStore}, '.aoPhases(',num2str(iPhase),')'],	'this.afPP(this.oMT.tiN2I.CO2)',        'Pa', [csStores{iStore}, ' CO2 Pressure']);
                oLog.addValue(['Example.toStores.', csStores{iStore}, '.aoPhases(',num2str(iPhase),')'],	'this.afPP(this.oMT.tiN2I.H2O)',        'Pa', [csStores{iStore}, ' H2O Pressure']);
                oLog.addValue(['Example.toStores.', csStores{iStore}, '.aoPhases(',num2str(iPhase),')'],	'fTemperature',	'K',  [csStores{iStore}, ' Temperature']);
            end
