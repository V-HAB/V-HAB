classdef PlantModuleExample < vsys
    ...
    methods
        function this = PlantModuleExample(oParent, sName)
            this@vsys(oParent, sName, 15);
            
            % Creating the crew_module store, volume 80m^3
            this.addStore(matter.store(this.oData.oMT, 'crew_module', 800000));
            
            % Creating the phase for the crew module using the 'air' helper
            oCrew_Module_Air = this.toStores.crew_module.createPhase('air', 800000,293,0.5);
%            %oCrew_Module_Air = matter.phases.gas(this.toStores.crew_module, ...
%                 'air', ... Phase name
%                 struct('O2',8*21.49/8*700,'N2',8*70.35/8*700,'CO2',8*0.09/8*700,'H2O',10), ... Phase contents - set by helper
%                 8*7000, ... Phase volume
%                 293.15);
           ...
           % this.oProc_Manip = hami.BIO_LSS.components.food_Reactor('food_Reactor', oFoodStorage);
            ...
            oHygieneWaterPhase = matter.phases.liquid(this.toStores.HygieneWaterTank, ...   Store in which the phase is located
                ...
                struct('H2O', 100000), ...      Phase contents
                100000, ...                 Phase volume
                293.15, ...             Phase temperature
                101325);              % Phase pressure
            ...
            %algae_module:
            %oSubSysAlgaeModule = hami.BIO_LSS.subsystems.algae_module(this, 'SubSystemAlgaeModule');
