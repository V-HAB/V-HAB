function ParallelExecution(cmInputs, csSimulationNames)
    %% ParallelExecution
    % this script can be used to start parallel execution of ISS
    % simulations with different parameters. This is usefull if e.g. a case
    % study for multiple different cases shall be run on a cluster without
    % GUI in parallel. The parameters can be defined as inputs for this
...
%                   containers.Map({'tbCases', 'sPlantLocation', 'fSimTime'},{struct('ACLS', false, 'PlantChamber', true),  'SM',       300})};
    %
    % basically a cell array of potential inputs for the different cases
    % defined in the ISS simulation can be handed in. Refer to the comments
    % in the ISS setup file for further information on this.
    %
    ...
    sStorageDirectory = [ datestr(fCreated, 'yyyy-mm-dd_HH-MM-SS_FFF') '_ParallelISSExecution'];
    ...
    for  iSimulation = 1:iSimulations
        ...
        aoResults(iSimulation) = parfeval(oPool, @runSim, 1, mInputs, oMT, aoDataQueues(iSimulation), iSimulation);  %#ok
...
function oLastSimObj = runSim(mInputs, oMT, oDataQueue, iSim)
    ...
    oLastSimObj = vhab.sim('simulations.ISS.setup', containers.Map(csKeys, csValues), []);
