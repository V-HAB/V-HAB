classdef SWMEStore < matter.store
    ...
    %  membrane in this case, roth.swme.procs.X50Membrane)
    
    % SWME(oMT, sName, fVolume, tParam)
    ...
    properties (SetAccess = protected, GetAccess = public)
        oProc;                          % P2P processor
        fSWMELiquidVolume;              % volume available for the liquid phase inside the hollow fibers
    end
    
    methods
        
        function this = SWMEStore(oContainer, sName, tParam)
            
            this@matter.store(oContainer, sName, tParam.fSWMEVolume);
            
            %defining volume available for the liquid phase inside the
            %HoFis
            this.fSWMELiquidVolume = (tParam.fSWMEVolume - tParam.fSWMEVaporVolume);
            
            % creating liquid water phase inside the hollow fibers of the
            % X50 membrane
            oLiquidHoFiPhase = matter.phases.liquid(this,...                 % Store where the phase is located
                                         'HoFiWater', ...                    % Phase name
                                          struct('H2O', 0.0881439), ...      % Phase contents
                                          this.fSWMELiquidVolume, ...        % Phase volume
                                          tParam.fInitialInletTemperature,...% Phase temperature 
                                          28300);                            % Phase pressure
                                      
            % creating the vapor phase filling the SWME around the hollow
            % fibers
            oVaporSWME = matter.phases.gas(this, ...                         % Store in which the phase is located
                                         'VaporSWME', ...                    % Phase name
                                          struct('H2O', 9.160756e-6), ...    % Phase contents
                                          tParam.fSWMEVaporVolume, ...        % Phase volume
                                          tParam.fInitialInletTemperature);  % Phase temperature
            ...
            this.oProc = roth.swme.procs.X50Membrane(this, 'X50Membrane', 'HoFiWater.WaterToVapor', 'VaporSWME.VaporIn');
