...
classdef SilicaGel_Table < handle
    ...
    properties  
        ...
        fR_pellet = 1.125*10^-3;                             % pellet radius [m]
        ...
        k_l = [0.00125, 0, 0, 0];         % LDF model kinetic lumped constant in order (H2O, CO2, N2, O2) [1/s]
    ...
    methods
        function this = SilicaGel_Table(k_l)         
            % Initialize the toth istotherm constants Values taken from:
            % 'Full System Modeling and Validation of the Carbon Dioxide
            % Removal Assembly' (ICES-2014-168)
            
                                     % CO2       ; H2O       ; O2 ; N2 
            this.mfTothParamter_a0 = [ 7.678E-6  ; 1.767E2   ; 0  ; 0  ]; 
            this.mfTothParamter_b0 = [ 5.164E-7  ; 2.787E-5  ; 0  ; 0  ];
            this.mfTothParamter_E  = [ 2.330E3   ; 1.093E3   ; 0  ; 0  ];
            this.mfTothParamter_t0 = [ -3.053E-1 ; -1.190E-3 ; 0  ; 0  ];
            this.mfTothParamter_c0 = [ 2.386E2   ; 2.213E1   ; 0  ; 0  ];
            
            % Conversion from [1/kPa] to [1/Pa]
            this.mfTothParamter_a0 = this.mfTothParamter_a0./1000;
            this.mfTothParamter_b0 = this.mfTothParamter_b0./1000;
        ...
        function [q_equ] = calculate_q_equ(this, afC_in, fTemperature, fRhoSorbent, csNames, ~)
            ...
            if(size(fTemperature)==1)  % homogeneours temperature throughout bed
                a = this.mfTothParamter_a0.*exp(this.mfTothParamter_E/fTemperature);
                b = this.mfTothParamter_b0.*exp(this.mfTothParamter_E/fTemperature);
                t_T = this.mfTothParamter_t0 + this.mfTothParamter_c0/fTemperature;
            else    % temperature varies along length of bed
                a = this.mfTothParamter_a0.*exp(this.mfTothParamter_E/fTemperature);
                b = repmat(this.mfTothParamter_b0,1,size(fTemperature,2)).*exp(repmat(this.mfTothParamter_E,1,size(fTemperature,2))./repmat(fTemperature,length(csNames),1));
                t_T = repmat(this.mfTothParamter_t0,1,size(fTemperature,2)) + repmat(this.mfTothParamter_c0,1,size(fTemperature,2))./repmat(fTemperature,length(csNames),1);
            end
            summe = sum(repmat(b,1,size(p_i,2)).*p_i,1);
            
            q_equ = zeros(size(p_i));
            for iVar_2 = 1:length(csNames)
                q_equ(iVar_2,:) = a(iVar_2)*p_i(iVar_2,:) ./ ((1+summe.^t_T(iVar_2)).^(1/t_T(iVar_2)));
            end
