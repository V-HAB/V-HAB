...
% for JAXA's CDR:
% Change values in this code
%  - Geometry: L = 0.3, D = 0.06, e_b = 0.4;
%  - feed: p = 127054, T = 300, c_in = 3810, V = 2*10^-4 ( = 2.84e-4 [kg/s] )
%  - Flow: 'N2',  0.99404; 'CO2', 0.00596


classdef FBA_Table < handle
    ...
    properties  
        ...
        % Constants
        mfToth_table;                                       % toth istotherm constants
    ...
    methods
        function this = FBA_Table()         
            ...
            % Check if JAXA's CDR simulation is desired and set corresponding constants        
            if this.bCDR == true
                this.D_L = 2.432E-03;               % [m^2/s]
                this.fR_pellet = 1.7*10^-3;         % [m]
                this.fMacroporeTurt_tau = 5;        % [-]
                this.fR_macropore = 3.2*10^-7;      % [m]
                % In the order (H2O, CO2, N2, O2) 
                this.k_l = [0, 1.190e-03, 6.412e-03, 0];    % [1/s]
            end        
        end
