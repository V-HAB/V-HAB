...
if strcmpi(sHX_type, 'counter annular passage')
    ...
    %help)
    falpha_o = functions.heat_transfer_coefficient.calculateConvectionAnnularPassage (fD_i, fD_o, fLength,...
    ...
    falpha_pipe = functions.heat_transfer_coefficient.calculateConvectionPipe ((2*fR_i), fLength, fFlowSpeed_Fluid1,...
    ...
    fR_lambda = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid, 0, fR_i,...
                (fD_i/2), fLength);
   ...
   %in case that condensation does occur this is the lower limit for the
    ...
    fDelta_P_1 = functions.pressure_loss.calculatePipe((2*fR_i), fLength, fFlowSpeed_Fluid1,...
                    fDyn_Visc_Fluid1, fDensity_Fluid1, 0);
    fDelta_P_2 = functions.pressure_loss.calculatePipe((fD_o - fD_i), fLength,...
...
elseif strcmpi(sHX_type, 'counter plate')
    ...
    %function help)
    falpha_1 = functions.heat_transfer_coefficient.calculateConvectionPlate (fLength, fFlowSpeed_Fluid1,...
    ...
    %function help)
    falpha_2 = functions.heat_transfer_coefficient.calculateConvectionPlate (fLength, fFlowSpeed_Fluid2,...
                    fDyn_Visc_Fluid2, fDensity_Fluid2,...
                    fThermal_Cond_Fluid2, fC_p_Fluid2);
    
    %uses the function for thermal resisitvity to calculate the resistance 
    %from heat conduction (for further information view function help)    
    fR_lambda = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid, 2, fArea,...
                    fThickness); 
   ...
   %in case that condensation does occur this is the lower limit for the
    ...
    %calculation of pressure loss for both fluids:
    fDelta_P_1 = functions.pressure_loss.calculatePipe(((4*fBroadness*fHeight_1)/...
                  (2*fBroadness+2*fHeight_1)) , fLength,...
                  fFlowSpeed_Fluid1, fDyn_Visc_Fluid1, fDensity_Fluid1, 0);
    fDelta_P_2 = functions.pressure_loss.calculatePipe(((4*fBroadness*fHeight_1)/...
...
%     end
    ...
    if fEntry_Temp1 > fEntry_Temp2
        ...
        fFlowSpeed_Fluid2_Nominal = (600*0.000125997881)/(((pi*(fD_o/2)^2)-(pi*(fD_i/2)^2))*...
                        fDensity_Fluid2(1));   
        falpha_o_Nominal = functions.heat_transfer_coefficient.calculateConvectionAnnularPassage (fD_i, fD_o, fLength,...
...
elseif strcmpi(sHX_type, 'counter pipe bundle')
    ...
    falpha_o = functions.heat_transfer_coefficient.calculateConvectionPipe(fOuter_Hydraulic_Diameter, fLength,...
                fFlowSpeed_Fluid2, fDyn_Visc_Fluid2, fDensity_Fluid2,...
                fThermal_Cond_Fluid2, fC_p_Fluid2, 0);
    
    %uses the function for convection in a pipe to calculate the inner
    %convection coeffcient (for further information view function help)
    falpha_pipe = functions.heat_transfer_coefficient.calculateConvectionPipe (fD_i, fLength, fFlowSpeed_Fluid1,...
                    fDyn_Visc_Fluid1, fDensity_Fluid1,...
                    fThermal_Cond_Fluid1, fC_p_Fluid1, 0);

    %uses the function for thermal resisitvity to calculate the resistance 
    %from heat conduction (for further information view function help)
    fR_lambda = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid, 0, (fD_i/2),...
                    (fD_o/2), fLength);
    ...
    %calculation of pressure loss for both fluids:
    fDelta_P_1_OverPipe = functions.pressure_loss.calculatePipe(fD_i , fLength,...
                            fFlowSpeed_Fluid1, fDyn_Visc_Fluid1,...
                            fDensity_Fluid1, 0);
    fDelta_P_1_InOut = functions.pressure_loss.calculatePipeBundle_InletOutlet(fD_i, fs_1, fs_2,...
                            fFlowSpeed_Fluid1, fDyn_Visc_Fluid1,...
                            fDensity_Fluid1);
    
    fDelta_P_1 = fDelta_P_1_OverPipe + fDelta_P_1_InOut;
    
    fDelta_P_2 = functions.pressure_loss.calculatePipe(fOuter_Hydraulic_Diameter, fLength,...
...
elseif strcmpi(sHX_type, 'parallel annular passage')
    ...
    %calculation of pressure loss for both fluids:
    fDelta_P_1 = functions.pressure_loss.calculatePipe((2*fR_i), fLength,...
                    fFlowSpeed_Fluid1, fDyn_Visc_Fluid1,...
                    fDensity_Fluid1, 0);
    fDelta_P_2 = functions.pressure_loss.calculatePipe((fD_o - fD_i), fLength,...
                    fFlowSpeed_Fluid2, fDyn_Visc_Fluid2,...
...
elseif strcmpi(sHX_type, 'parallel plate')
    ...
    falpha_o = functions.heat_transfer_coefficient.calculateConvectionPlate (fLength, fFlowSpeed_Fluid1,...
                fDyn_Visc_Fluid1, fDensity_Fluid1, fThermal_Cond_Fluid1,...
                fC_p_Fluid1);
    falpha_i = functions.heat_transfer_coefficient.calculateConvectionPlate (fLength, fFlowSpeed_Fluid2,...
                    fDyn_Visc_Fluid2, fDensity_Fluid2,...
                    fThermal_Cond_Fluid2, fC_p_Fluid2);
  
    %uses the function for thermal resisitvity to calculate the resistance 
    %from heat conduction (for further information view function help)    
    fR_lambda = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid, 2, fIncrementalArea,...
                    fThickness);
...
elseif strcmpi(sHX_type, 'cross')
    ...
    if fN_Rows > 0
        ...
        %partially shiffted
        if fConfig == 0
            fDelta_P_2 = functions.pressure_loss.calculatePipeBundle(fD_o, fs_1, fs_2,...
    ...
    if fN_Rows == 0
        ...
        falpha_pipe = functions.heat_transfer_coefficient.calculateConvectionPlate(fLength, fFlowSpeed_Fluid2,...
                        fDyn_Visc_Fluid2, fDensity_Fluid2,...
                        fThermal_Cond_Fluid2, fC_p_Fluid2);
        ...
        %function help)    
        fR_lambda_Incremental = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid, 2, fIncrementalArea,...
    ...
    elseif fN_Rows == 1
        ...
        %function help)
        falpha_o = functions.heat_transfer_coefficient.calculateConvectionOnePipeRow(fD_o,fs_1, fFlowSpeed_Fluid2,...
        ...
        fR_lambda_Incremental = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid,0, (fD_i/2),...
            (fD_o/2), fIncrementalLength);
    ...
    elseif fN_Rows > 1
        ...
        if fConfig == 2
            falpha_o = functions.heat_transfer_coefficient.calculateConvectionMultiplePipeRow (fD_o, fs_1, fs_2,...
                        fFlowSpeed_Fluid2, fDyn_Visc_Fluid2,...
...
elseif strcmpi(sHX_type, '1 n sat')
    ...
    if (fHeat_Capacity_Flow_1 == 0) || (fHeat_Capacity_Flow_2 == 0)
        ...
        for k = 1:fP
            ...
            malpha_o(k,1) = functions.heat_transfer_coefficient.calculateConvectionPipe(mD_Hydraulic_Pass(k), fLength,...
                            fFlowSpeed_Fluid2, fDyn_Visc_Fluid2,...
                            fDensity_Fluid2, fThermal_Cond_Fluid2,...
                            fC_p_Fluid2, 0);
            ...
            %pass
            malpha_i(k,1) = functions.heat_transfer_coefficient.calculateConvectionPipe (mD_i(k,1), fLength,...
            ...
            fR_lambda(k,1) = functions.heat_transfer_coefficient.calculateConductionResistance(fThermal_Cond_Solid, 0,...
    ...
    fDelta_P_1_InOut = 0;
    for k = 1:fP
        fDelta_P_1_OverPipe = fDelta_P_1_OverPipe + functions.pressure_loss.calculatePipe(mD_i(k) , fLength, mw_fluid1(k), fDyn_Visc_Fluid1, fDensity_Fluid1, 0);
    
        fDelta_P_1_InOut = fDelta_P_1_InOut + functions.pressure_loss.calculatePipeBundle_InletOutlet(mD_i(k), fs_1, fs_2, mw_fluid1(k), fDyn_Visc_Fluid1, fDensity_Fluid1);
...
elseif strcmpi(sHX_type, '3 2 sat')
    ...
    if length(mHX)==21
        falpha_sheath = functions.heat_transfer_coefficient.functions.heat_transfer_coefficient.calculateConvectionSheathCurrent (fD_o, fD_Baffle,...
    ...
    fDelta_P_2 = functions.pressure_loss.calculateSheathCurrent(fD_o, fD_Shell, fD_Baffle,...
