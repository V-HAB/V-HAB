...
% phase, store and system in which it occurs for the CURRENT TICK!
    ...
    afMassUpdateTimeStep_in_Phases = reshape([ oLastSimObj.oSimulationContainer.oMT.aoPhases.fMassUpdateTimeStep ], [1,iNumberOfPhases]);
    
    miMinTimeStepIndices = find(afTimeStep_in_Phases == min(afTimeStep_in_Phases(afTimeStep_in_Phases ~= 0)));

    miMinMassUpdateTimeStepIndices = find(afMassUpdateTimeStep_in_Phases == min(afMassUpdateTimeStep_in_Phases));
...
%     disp(' ')
%     disp('The smallest mass updatetime steps for the current tick occured in:') 
%     for iI = 1:length(miMinMassUpdateTimeStepIndices)
%         % returns the system name, store name and phase name for the location
%         % where the highes mass losses occured
%         disp(['The system ', aoPhases(miMinMassUpdateTimeStepIndices(iI)).oStore.oContainer.sName, ' in Store ', aoPhases(miMinMassUpdateTimeStepIndices(iI)).oStore.sName, ' in Phase ', aoPhases(miMinMassUpdateTimeStepIndices(iI)).sName, ' a minimal mass update time step of ' num2str(aoPhases(miMinMassUpdateTimeStepIndices(iI)).fTimeStep), ' was used'])
%         
%     end
end
