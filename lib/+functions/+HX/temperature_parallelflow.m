%Returns the outlet temperatures of a parallelflow heat exchanger in K
%
...
%
%[fOutlet_Temp_c, fOutlet_Temp_h] = temperature_parallelflow (fArea, fU,...
%                         fHeat_Cap_Flow_c, fHeat_Cap_Flow_h, fT1_0, fT2_0)
%
%Output [fOutlet_Temp_c, fOutlet_Temp_h] in K

function [fOutlet_Temp_c, fOutlet_Temp_h] = temperature_parallelflow ...
         (fArea, fU, fHeat_Cap_Flow_c, fHeat_Cap_Flow_h, fEntry_Temp_c,...
          fEntry_Temp_h)

%the source "Wärmeübertragung" Polifke will from now on be defined as [1]

%Number of Tranfer Units
...
if fHeat_Cap_Flow_c < fHeat_Cap_Flow_h
    ...
    %calculates theta_c the dimensionless temperature for the cold fluid
    fTheta_c = (1-exp(-fNTU*(1+fHeat_Cap_Flow_Ratio)))/...
               (1+fHeat_Cap_Flow_Ratio);            
    %Equation from [1] page 187 table 8.1
    
    %calculates fTheta_h, the dimensionless temperature for the hot fluid
    %from fTheta_c and fHeat_Cap_Flow_Ratio
    fTheta_h = fHeat_Cap_Flow_Ratio*fTheta_c;                           
    %Equation from [1] page 176 equation (8.10)
    ...
    %calculates fTheta_c, the dimensionless temperature for the cold fluid
    %from fTheta_h and fHeat_Cap_Flow_Ratio
